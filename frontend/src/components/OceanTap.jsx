...
export default function OceanTap({ active, onToggle }) {
  ...
  const dropId = useState(0);

  // Spawn drops when active
  useEffect(() => {
    if (!active) { setDrops([]); return; }
    const interval = setInterval(() => {
      setDrops(prev => [...prev.slice(-5), { id: Date.now() }]);
    }, 380);
  ...
  // Match PullSwitch color palette
  const pipeColor   = isDark ? "#3a3a4a" : "#b0a8c8";
  const pipeShade   = isDark ? "#2a2a3a" : "#9888b8";
  const handleFill  = active
    ? (isDark ? "#7dd3fc" : "#38bdf8")
    : (isDark ? "#c4b0f0" : "#9880cc");
  const dropColor   = isDark ? "#7dd3fc" : "#38bdf8";
  const glowColor   = active ? "rgba(56,189,248,0.45)" : "transparent";

  return (
    <div
      onClick={handleClick}
      title={active ? "Turn off ocean mode" : "Dive into the ocean 🌊"}
      style={{
        position: "relative",
        width: 44,
        height: 58,
    ...
    >
      <style>{`
        @keyframes ot-drop {
          ...
          60%  { transform: translateY(28px) scale(0.9); opacity: 0.6; }
      ...
      >
        {/* ── Ceiling mount — matches PullSwitch bracket ── */}
        ...
        {/* ── Tap handle (horizontal lever) — like the pull ring ── */}
        {/* Lever base */}
        <rect x="28" y="27" width="12" height="4" rx="2" fill={handleFill} />
        {/* Lever knob — circular like the pull ring */}
        <circle cx="40" cy="29" r="4.5" fill="none" stroke={handleFill} strokeWidth="2" />
        <circle cx="40" cy="29" r="2.2" fill={handleFill} />

        {/* ── Water glow when active ── */}
        {active && (
          <ellipse cx="13" cy="52" rx="5" ry="3" fill={glowColor} />
        )}
      </svg>

      {/* ── Animated drops — same position as bulb hangs ── */}
      {drops.map(drop => (
        <div
          key={drop.id}
          style={{
            ...
            background: `linear-gradient(180deg, ${dropColor}cc, ${dropColor})`,
            animation: "ot-drop 0.85s ease-in forwards",
            pointerEvents: "none",
          }}
        />
      ))}
