...
export default function PullSwitch({ inline = false }) {
  ...
  const ex   = 50 + Math.sin(rad) * CORD;   // cord end x
  const ey   = 8  + Math.cos(rad) * CORD;   // cord end y
    ...
    <>
      ...
      {flash && (
        <div style={{
          ...
          background: "rgba(255,250,180,0.15)",
          animation: "ps-flash 0.35s ease-out forwards"
        }} />
      )}

      <style>{`
        @keyframes ps-flash { from { opacity:1 } to { opacity:0 } }
      `}</style>
      ...
      >
        <svg
          ...
          style={{
            ...
            filter: isDark
              ? "drop-shadow(0 0 10px rgba(255,240,80,0.3)) drop-shadow(0 4px 8px rgba(0,0,0,0.4))"
        ...
        >
          ...
          {/* Cord */}
          <line
            x1="50" y1="9" x2={ex} y2={ey}
            stroke={cordColor} strokeWidth="2.4" strokeLinecap="round"
          />
          ...
          {/* Glow halo (dark only) */}
          {isDark && (
            <ellipse cx={ex} cy={ey+30} rx="18" ry="20" fill={glowColor} />
          )}
          ...
          {/* Filament */}
          <path
            d={`M${ex-4} ${ey+24} Q${ex} ${ey+19} ${ex+4} ${ey+24} Q${ex} ${ey+29} ${ex-4} ${ey+33}`}
            fill="none"
            stroke={filament}
            strokeWidth="1.3"
            strokeLinecap="round"
