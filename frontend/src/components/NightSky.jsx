    ...
    if (theme === "light") {
      ...
      const drawCloud = (cw, ch, cloud) => {
        ...
        for (let p = 0; p < cloud.puffs; p++) {
          const px = x + p * puffW + puffW * 0.5;
          const py = y - h * (0.3 + Math.sin(p * 1.2) * 0.2);
          ctx.beginPath();
          ctx.arc(px, py, puffW * 0.52, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.beginPath();
        ctx.roundRect(x, y - h * 0.3, w, h * 0.7, h * 0.3);
        ctx.fill();
        ctx.restore();
      };
      ...
      const draw = () => {
        ...
        // Sky gradient — soft, not saturated
        const sky = ctx.createLinearGradient(0, 0, 0, ch);
        sky.addColorStop(0,    "#fde8c8");
        sky.addColorStop(0.25, "#fcd4a0");
        sky.addColorStop(0.50, "#fac898");
        sky.addColorStop(0.72, "#fde0b0");
        sky.addColorStop(0.88, "#fef0d4");
        ...
        hGlow.addColorStop(0,   "rgba(255,210,140,0.12)");
        hGlow.addColorStop(0.4, "rgba(255,180,80,0.04)");
        ...
        // Sun
        const sunX = cw * 0.5;
        const sunY = ch * (0.64 + Math.sin(t * 0.0003) * 0.012);
        const sunR = Math.min(cw, ch) * 0.048;

        // Rays — faint
        rays.forEach(ray => {
          const len  = ray.len * Math.min(cw, ch);
          const x2   = sunX + Math.cos(ray.angle) * len;
          const y2   = sunY + Math.sin(ray.angle) * len;
          const rg   = ctx.createLinearGradient(sunX, sunY, x2, y2);
          rg.addColorStop(0, `rgba(255,220,80,${ray.alpha})`);
          rg.addColorStop(1, "transparent");
          ctx.beginPath();
          ctx.moveTo(sunX, sunY);
          ctx.lineTo(x2, y2);
          ctx.strokeStyle = rg;
          ctx.lineWidth = ray.width * 0.65;
          ctx.stroke();
        });

        // Halo
        const halo = ctx.createRadialGradient(sunX, sunY, sunR * 0.5, sunX, sunY, sunR * 2.6);
        halo.addColorStop(0,   "rgba(255,240,180,0.14)");
        halo.addColorStop(0.4, "rgba(255,210,120,0.05)");
        ...
        clouds.forEach(cloud => {
          cloud.x += cloud.speed;
          if (cloud.x > 1.5) cloud.x = -0.4;
          drawCloud(cw, ch, cloud);
        });

        // Ground fog
        const fog = ctx.createLinearGradient(0, ch * 0.92, 0, ch);
        fog.addColorStop(0, "transparent");
        fog.addColorStop(1, "rgba(255,245,220,0.28)");
        ctx.fillStyle = fog;
