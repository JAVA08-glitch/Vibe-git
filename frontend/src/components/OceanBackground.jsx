...
export default function OceanBackground({ active }) {
  ...
  const depthRef  = useRef(0); // 0 = surface, 1 = deep
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const d  = depthRef.current; // 0..1
      t++;

      // ── Sky gradient blending into ocean ──
      const sky = ctx.createLinearGradient(0, 0, 0, ch);

      if (d < 0.15) {
        // Surface: sky → light ocean
        const p = d / 0.15;
      ...
      if (rayOpacity > 0.005) {
        for (let i = 0; i < 6; i++) {
          ...
          rg.addColorStop(0,   `rgba(180,230,255,${rayOpacity})`);
      ...
      const causticOpacity = Math.max(0, 0.8 - d * 3) * 0.06;
      if (causticOpacity > 0.002) {
        for (let i = 0; i < 8; i++) {
          const cx2 = cw * (0.05 + i * 0.13 + Math.sin(t * 0.008 + i * 1.3) * 0.06);
          const cy2 = ch * (0.05 + Math.cos(t * 0.006 + i * 0.9) * 0.04);
          const cr  = ctx.createRadialGradient(cx2, cy2, 0, cx2, cy2, 80);
          cr.addColorStop(0,   `rgba(200,240,255,${causticOpacity})`);
      ...
      if (bubbleOpacity > 0.01) {
        bubbles.forEach(b => {
          ...
          const bx = (b.x + Math.sin(b.wobble) * 0.015) * cw;
          const by = b.y * ch;
          ctx.beginPath();
          ctx.arc(bx, by, b.r, 0, Math.PI * 2);
          ctx.strokeStyle = `rgba(180,230,255,${bubbleOpacity * 0.5})`;
          ctx.lineWidth = 0.8;
          ctx.stroke();
          // highlight
          ctx.beginPath();
      ...
      vig.addColorStop(1,   `rgba(0,10,30,${d * 0.55})`);
      ctx.fillStyle = vig;
      ctx.fillRect(0, 0, cw, ch);
  ...
  // Animate depth when active changes
  useEffect(() => {
